...
| Role       | Description |
|-----------|-------------|
| **Patient** | Barangay resident using the platform for symptom check, triage guidance, and referrals. |
| **BHW**     | Barangay Health Worker: conducts assisted intake, registers patients, views triage and referrals for their barangay. |
| **Clinician** | Nurse/doctor: validates triage, creates referrals, and reviews patient history and visit history for assigned patients. |
...
- **Own data only:** profile, patient_profile, medical_history, consent_records, symptom_assessments (own), ai_triage_results (for own assessments), referrals (where `patient_id` = self), notifications (own).
...
- **Barangay-scoped:** patient_profiles, symptom_assessments, ai_triage_results, referrals for patients in their `assigned_barangay_id`. Can create referrals (from_provider_id = self).  
...
- **Own:** profile.  
- **Assigned patients:** patients linked through clinician workflows (triage/referrals) and referral ownership (`from_provider_id` = self); for those patients: patient_profiles, medical_histories, symptom_assessments, ai_triage_results, referrals, visit history (triage + referrals).  
...
- **Full read** (subject to RLS) for reporting and oversight: profiles, barangays, patient_profiles, medical_histories, symptom_assessments, ai_triage_results, referrals, audit_logs, notifications, research_exports, etc.  
...
- **profiles, patient_profiles, medical_histories, symptom_assessments, ai_triage_results:** Policies should restrict reads/writes so that patients see only own data, BHWs only barangay-scoped data, clinicians only data for their assigned patients, and admin has the access needed for dashboard/analytics/audit (often via service role or explicit admin policies).  
...
- **Patient:** Own records and referrals only.  
- **BHW:** Own + barangay patients (assigned_barangay_id).  
- **Clinician:** Own + assigned patients (via triage/referrals and visit history).  
